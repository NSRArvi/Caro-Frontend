...
import 'package:jimamuapp/modules/place_order/location_picker_screen.dart';
...
class PlacingOrderController extends GetxController {
  ...
  void onInit() {
    ...
    // receiverSelectedCountryName.value = user.country ?? "Canada";
  ...
  locationStepConfirm() async {
    ...
    if (!(await isSameCountry())) {
      AppSnackbar.error(
        "Pickup and delivery must be in the same country!",
        // backgroundColor: AppColors.primaryColor.withOpacity(0.25),
      );
      return;
    }
    ...
    if (infoFormKey.currentState!.validate()) {
      if (additionalInfoKey.currentState!.validate()) {
        if (selectedPackage.value != null) {
          ...
          log("getting response $response");
          final data = response['data']['national'];
  ...
  placeOrder() async {
    ...
    try {
      CustomLoading.loadingDialog();
      final response = await ApiManager.placeOrder(
        placeOrderRequest,
